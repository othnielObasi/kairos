 ...
 *     Uses the same AIsa feed labels as the live runtime
 *
...
async function main() {
  ...
  for (let cycle = 1; cycle <= cycleCount; cycle++) {
    ...
    for (const event of CYCLE_EVENTS) {
      ...
      console.log(`  ${trackLabel} ${isReal ? 'OK' : 'NO'} ${event.name.padEnd(28)} ${costStr}  ${shortHash}`);
  ...
  console.log('  payment amount by 4,880x.');
  console.log();
