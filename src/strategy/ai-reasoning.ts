 ...
 * AI Reasoning Engine
 *
 * Provides human-readable explanations for each execution decision with a
 * production failover chain:
 *
 *   Gemini primary -> Gemini secondary -> GPT-4o mini -> Claude -> deterministic fallback
 *
 * The AI does not make the execution decision - the risk engine does.
...
export async function generateReasoning(
  ...
  if (openaiKey) {
    try {
      ...
      try { billingStore.addComputeEvent(await billEvent('compute-llm', { model: OPENAI_MODEL, type: 'inference' })); } catch (_) {}
      return result;
    } catch (error) {
      log.warn('OpenAI API failed - trying Claude', { error: String(error) });
    }
  }
  ...
  if (anthropicKey) {
    try {
      ...
      try { billingStore.addComputeEvent(await billEvent('compute-llm', { model: ANTHROPIC_MODEL, type: 'inference' })); } catch (_) {}
      return result;
    } catch (error) {
      log.warn('Claude API failed - using deterministic fallback', { error: String(error) });
    }
  }

  if (!anthropicKey && geminiKeys.length === 0 && !openaiKey) {
    log.debug('No LLM API keys set (GEMINI_API_KEY_PRIMARY / GEMINI_API_KEY_SECONDARY / GEMINI_API_KEY / OPENAI_API_KEY / ANTHROPIC_API_KEY) - using deterministic fallback');
  }
...
/**
 * Build the shared prompt used by Claude, Gemini, and OpenAI.
 */
function buildReasoningPrompt(
  ...
  if (sentiment && sentiment.sources.length > 0) {
    ...
    const fgLabel = fgRaw !== null
      ? (fgRaw <= 20 ? 'Extreme Fear' : fgRaw <= 40 ? 'Fear' : fgRaw <= 60 ? 'Neutral' : fgRaw <= 80 ? 'Greed' : 'Extreme Greed')
      : 'N/A';
...
- Checks passed: ${risk.checks.filter((check) => check.passed).length}/${risk.checks.length}
${risk.checks.filter((check) => !check.passed).map((check) => `- FAILED: ${check.name} - ${check.detail}`).join('\n')}
...
async function callGeminiAPI(
  ...
  log.info('AI reasoning generated [Gemini]', { model, summary: parsed.summary.slice(0, 80) });
  return parsed;
}
...
/**
 * Intelligent fallback - generates reasoning without an API call.
 */
function buildFallbackReasoning(
  ...
  if (!risk.approved) {
    const failed = risk.checks.filter((check) => !check.passed).map((check) => check.name);
    if (cb.active) {
  ...
  const passCount = risk.checks.filter((check) => check.passed).length;
  if (passCount === risk.checks.length) {
    ...
    riskNarrative = `${passCount}/${risk.checks.length} checks passed. ${risk.checks.filter((check) => !check.passed).map((check) => check.detail).join('. ')}`;
  }
  ...
  const summary = risk.approved
    ...
    : `${dir} signal rejected - ${risk.checks.filter((check) => !check.passed).map((check) => check.name).join(', ')}.`;
